...
filepath = basepath+'\\'+str(now_time)+'admin_pass'+'.txt'
...
res_date = browser.find_elements_by_xpath(".//*[@id='setTime']")
for r_date in res_date:
    r_date.send_keys("1")

res_pass = browser.find_elements_by_xpath(".//*[@id='page-wrapper']/div/div/div/div[3]/form/div/input[8]")
for r_pass in res_pass:
    r_pass.click()
...
for r_check1 in res_check1:
    ...
    if str(text) in r_check1.text:
        f = open(filepath,'w')
        f.writelines(u'The pass function is succeed')
    else:
        f = open(filepath,'w')
        f.writelines(u'The pass function is filed')
browser.close()