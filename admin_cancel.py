...
filepath = basepath+'\\'+str(now_time)+'admin_cancel'+'.txt'
...
res_pass = browser.find_elements_by_xpath(".//*[@id='page-wrapper']/div/div/div/div[3]/form/div/input[3]")
...
for r_check1 in res_check1:
    ...
    if str(text) in r_check1.text:
        f = open(filepath,'w')
        f.writelines(u'The cancel function is succeed')
    else:
        f = open(filepath,'w')
        f.writelines(u'The cancel function is filed')
browser.close()