...
res_end = browser.find_elements_by_xpath(".//*[@id='page-wrapper']/div/div/div/div[3]/form/div/input[9]")
...
for r_check1 in res_check1:
    ...
    if str(text) in r_check1.text:
        f = open(filepath,'w')
        f.writelines(u'The back function is succeed')
    else:
        f = open(filepath,'w')
        f.writelines(u'The back function is filed')
browser.close()